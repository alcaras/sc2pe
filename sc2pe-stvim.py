...
teams = {
    "ST" : ["Bomber", "Curious", "Squirtle", "TricKsteR", "July",
                  "RainBOw", "Virus", "FruitDealer", "Max", "Ace",
                  "Tiger", "Sound"],

    "IM" : ["NesTea", "MVP", "LoSirA", "Younghwa", "Happy",
            "Seed", "Yoda", "dreamertt", "BBi", "True",
            "Junwi"],
}
...
players = {
    ...
    # IM
    "Younghwa": ["P", 2127, {"T": 2062, "Z": 2005, "P": 2091}],
    "Seed": ["P", 2008, {"T": 2008, "Z": 1973, "P": 1956}],
    "dreamertt": ["T", 1963, {"T": 1887, "Z": 1993, "P": 2057}],
    "BBi": ["P", 1959, {"T": 1931, "Z": 2054, "P": 1980}],
    "True": ["Z", 1943, {"T": 1946, "Z": 1969, "P": 1998}],
    "Junwi": ["Z", 1869, {"T": 1935, "Z": 1963, "P": 1930}],

    # ST
    "Curious": ["Z", 2086, {"T": 2017, "Z": 2022, "P": 2072}],
    "Squirtle": ["P", 2056, {"T": 2039, "Z": 1991, "P": 2040}],
    "RainBOw": ["T", 2016, {"T": 1919, "Z": 1965, "P": 2118}],
    "Ace": ["P", 1946, {"T": 1994, "Z": 1954, "P": 1966}],
    "Tiger": ["P", 1943, {"T": 1943, "Z": 1986, "P": 2000}],
    "Sound": ["T", 1895, {"T": 1890, "Z": 1961, "P": 1978}],
    # placeholder as terran as we don't have R implemented
    "Max": ["T", 1969, {"T": 2000, "Z": 2000, "P":2000}],
...
def announce_match(map, A, B, s1=0, s2=0):
    matchup = get_matchup(A, B)
    npA = predict_result_on(map, A, B)
    if s1+s2 > 0:
...
def sim_gstl_rec(tA, tB, tM, winner, wP, sA=0, sB=0):
    if not tA or not tB:
        return 0
    ...
    if winner == "A":
        ...
        for m in tM:
            for p in tB:
                r = predict_result_on(m, wP, p)
                if r < min:
                    lowP = p
                    lowM = m
                    min = r
        probWin = announce_match(lowM, wP, lowP, sA, sB)
        winb = sim_gstl_rec(tA, ll(tB, lowP), ll(tM, lowM), "A", wP, sA+1, sB)
        loseb = sim_gstl_rec(ll(tA, wP), tB, ll(tM, lowM), "B", lowP, sA, sB+1)
    else:
        lowM = ""
        lowP = ""
        max = 0
        for m in tM:
            for p in tA:
                r = predict_result_on(m, p, wP)
                if r > max:
                    lowP = p
                    lowM = m
                    max = r
        probWin = announce_match(lowM, lowP, wP, sA, sB)
        winb = sim_gstl_rec(tA, ll(tB, wP), ll(tM, lowM), "A", lowP, sA+1, sB)
        loseb= sim_gstl_rec(ll(tA, lowP), tB, ll(tM, lowM), "B", wP, sA, sB+1)

    pA = probWin * winb + (1-probWin) * loseb
    return pA
    


# tA v. tB 
# match 1 is stA v. stB on stM
def sim_gstl_match(tA, tB, tM, stA, stB, stM):
    pWin = announce_match(stM, stA, stB)
    
    winB = sim_gstl_rec(tA, ll(tB, stB), ll(tM, stM), "A", stA, 1, 0)
    loseB = sim_gstl_rec(ll(tA, stA), tB, ll(tM, stM), "B", stB, 0, 1)

    print pWin * winB + (1-pWin)*loseB
...
# if bomber wins, 1-0 ST
IM = teams["IM"]
ST = teams["ST"]

sim_gstl_match(ST, IM, named_maps, "Bomber", "MVP", "Crevasse")
